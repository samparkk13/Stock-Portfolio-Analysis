...
def chat():
    ...
    if not user_message:
        return jsonify({'message': 'Please provide a message.', 'status': 'error'}), 400

    if not llm_with_tools:
        return jsonify({'message': 'Chatbot is not properly configured.', 'status': 'error'}), 500
    ...
    try:
        ...
        import re
        matches = re.findall(r'(\d+)\s+([A-Za-z]+)', user_message)
        if matches:
            session["portfolio"] = {ticker.upper(): int(shares) for shares, ticker in matches}
            print(f"[PORTFOLIO SAVED] {session['portfolio']}")
